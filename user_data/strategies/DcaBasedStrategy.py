...
class DcaBasedStrategy(IStrategy):
        ...
        try:
            ...
            if last_candle is not None and previous_candle is not None:
                ...
                if last_candle['close'] <= previous_candle['close'] \
                        or ((last_candle['adx'] <= previous_candle['adx']) and last_candle['adx'] < 25):
                    return None
    ...
    def load_dca_orders(self):
        try:
            ...
            if os.path.exists('user_data/dca_orders'):
                with open('user_data/dca_orders', 'rb') as handle:
    ...
    def save_dca_orders(self):
        try:
            with open('user_data/dca_orders', 'wb') as handle:
